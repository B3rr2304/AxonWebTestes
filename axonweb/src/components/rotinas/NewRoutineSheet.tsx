...
export default function NewRoutineSheet({ isOpen, onClose, onCreated }: Props) {
  ...
  function toggleDay(key: string, day: number) {
    setItems((cur) =>
      cur.map((it) => {
        ...
        const days = it.days.includes(day)
          ? it.days.filter((d) => d !== day)
          : [...it.days, day].sort((a, b) => a - b);
        return { ...it, days };
      })
    );
  }
  ...
  function removeItem(key: string) {
    setItems((cur) => (cur.length > 1 ? cur.filter((it) => it.key !== key) : cur));
  }
    ...
    <AnimatePresence>
      {isOpen && (
        <>
                ...
                <div className="space-y-3 py-2">
                  {items.map((it, idx) => (
                    <RoutineItemEditor
                      ...
                      onToggleDay={(d) => toggleDay(it.key, d)}
          ...
          {/* Modal de confirmação: rotina sem data de término */}
          <AnimatePresence>
            {showNoEndConfirm && (
              <motion.div
              ...
              >
                ...
                >
                  ...
                  <div className="mt-6 grid grid-cols-2 gap-3">
                    <button
                      type="button"
                      onClick={() => setShowNoEndConfirm(false)}
